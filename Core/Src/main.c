	...
	if (htim->Instance==TIM16){  // this is the 20ms timer
		...
		if (RM.controlType == CLOSED_LOOP){ 			//ClosedLoop
			if ((rampRPM.rampPhase!=RAMP_OVER)&&(rampRPM.rampPhase!=RAMP_WAIT)){
				...
				if (rampRPM.newLayerCalcAfterStop == 1){
					Recalculate_RampRPM_RampRates(&RM,&rampRPM,RM.transitionTarget);
					rampRPM.newLayerCalcAfterStop = 0;
				}
				S.motorState = IDLE_STATE;
				RM.logReturn=0;
				RM.logCycle = 0;
...
HAL_Init();
  ...
  while (1)
  {
	  ...
	  if (rampRPM.newLayerCalcAfterRampUp == 1){
		  if (rampRPM.rampPhase == RAMP_STEADY){
			  ChangeRPM(&RM,&rampRPM);
			  Recalculate_RampRPM_RampRates(&RM,&rampRPM,RM.transitionTarget);
			  rampRPM.rampPhase = RAMP_CHANGE;
			  rampRPM.newLayerCalcAfterRampUp = 0;
		  }
	  }
	  ...
	  if (S.CAN_MSG == CHANGE_RPM){
		  if(S.motorState == RUN_STATE){
			  ...
			  if (RM.controlType == CLOSED_LOOP){
				  // if we re ramping down or ramping up, when we receive the msg,
				  // we need to apply the change target differently. if ramping down , we ust have to save the
				  // new final target to be used after we come to a halt.
				  // if we re ramping up, if we re more than the target speed then we need to slow down,
				  //and if we re less than the target speed we need to go to that new target speed.
				  if (rampRPM.rampPhase == RAMP_DOWN){
					  rampRPM.newLayerCalcAfterStop = 1;
				  }else if (rampRPM.rampPhase == RAMP_UP){
					  rampRPM.newLayerCalcAfterRampUp = 1;
				  }else{
					  ChangeRPM(&RM,&rampRPM);
					  Recalculate_RampRPM_RampRates(&RM,&rampRPM,RM.transitionTarget);
					  rampRPM.rampPhase = RAMP_CHANGE;
